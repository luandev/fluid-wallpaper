...
DEC-019 adds `pack-wallpaper.mjs`: after a production Vite build, copy `dist/wallpaper.html` + referenced assets, host manifests from `wallpaper/`, and zip `fluid-wallpaper-desktop-<version>.zip`. Invoked by `yarn pack:wallpaper`. Does not upload Workshop or change Yarn-only installs.
