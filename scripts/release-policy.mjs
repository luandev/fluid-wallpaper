...
export function releasePolicy(version, ref, changelog) {
  ...
  assert(new RegExp(`^${number}\\.${number}\\.${number}(?:-${identifier}(?:\\.${identifier})*)?$`).test(version), "Invalid version (build metadata is not supported)");
  assert.equal(ref, `refs/tags/v${version}`, "Tag must match package version");
  const parts = changelog.replace(/\r\n/g, "\n").split(`## ${version}\n`);
  assert.equal(parts.length, 2, "Expected exactly one version changelog section");
