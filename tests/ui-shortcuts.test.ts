...
describe("shortcuts", () => {
  it("maps H P F Esc", () => {
    ...
    expect(shortcutFromKey({ code: "Escape", key: "Escape" }, null)).toBe(
      "exitFullscreen",
    );
  });

  it("ignores typing targets and repeats", () => {
    expect(shortcutFromKey({ code: "KeyH" }, { tagName: "INPUT" })).toBe(
      "ignore",
    );
    expect(shortcutFromKey({ code: "KeyP" }, { tagName: "TEXTAREA" })).toBe(
      "ignore",
    );
    expect(shortcutFromKey({ code: "KeyF", repeat: true }, null)).toBe(
      "ignore",
    );
