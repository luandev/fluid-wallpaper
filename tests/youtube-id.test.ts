...
describe("YouTube id parse", () => {
  ...
  it("accepts youtu.be, shorts, and embed paths", () => {
    expect(parseYouTubeId("https://youtu.be/wKEeVPfK8nw")).toBe(DEFAULT_YOUTUBE_ID);
    expect(parseYouTubeId("https://www.youtube.com/shorts/wKEeVPfK8nw")).toBe(DEFAULT_YOUTUBE_ID);
    expect(parseYouTubeId("https://www.youtube-nocookie.com/embed/wKEeVPfK8nw")).toBe(DEFAULT_YOUTUBE_ID);
  });

  it("rejects non-YouTube URLs", () => {
    expect(parseYouTubeId("https://example.com/watch?v=wKEeVPfK8nw")).toBeUndefined();
  ...
  it("canonicalizes stored URLs", () => {
    expect(sanitizeYoutubeUrl("https://youtu.be/wKEeVPfK8nw?t=12")).toBe(DEFAULT_YOUTUBE_URL);
    expect(youtubeEmbedSrc(DEFAULT_YOUTUBE_ID)).toContain(DEFAULT_YOUTUBE_ID);
  });

  it("sanitizes config URLs and clamps videoReveal", () => {
    const next = sanitizeConfig({ youtubeUrl: "https://youtu.be/wKEeVPfK8nw", videoReveal: 4 });
    expect(next.youtubeUrl).toBe(DEFAULT_YOUTUBE_URL);
    expect(next.videoReveal).toBe(1);
    expect(sanitizeConfig({ youtubeUrl: "https://example.com/watch?v=nope" }).youtubeUrl).toBe("");
