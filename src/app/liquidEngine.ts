import { assertSameCarriers, validateLiquidScene, type LiquidScene } from "./liquidScene";
...
import { LIQUID_TIERS, LiquidClock, LiquidQualityController } from "../quality/liquidQuality";
import { GpuTiming, type LiquidTiming } from "../quality/gpuTiming";

export interface LiquidStatus {
  requestedFps: number; effectiveFps: number;
  simulation: [number, number]; pigment: [number, number]; display: [number, number];
...
export class LiquidEngine {
  ...
  constructor(private canvas: HTMLCanvasElement, scene: LiquidScene, private onStatus: (status: LiquidStatus) => void, private onError: (error: Error) => void) {
    this.scene = validateLiquidScene(scene); this.quality.setPolicy(this.scene.quality);
    this.gl = createGl(canvas);
    try { this.initialize(); } catch (error) { this.release(); throw error; }
    const signal = this.abort.signal;
    document.addEventListener("visibilitychange", () => this.schedule(), { signal });
    canvas.addEventListener("webglcontextlost", e => {
      e.preventDefault(); this.lost = true; this.stop();
    ...
    canvas.addEventListener("webglcontextrestored", () => {
      try {
        this.release(); this.initialize(); this.lost = false;
        this.clock = new LiquidClock(); this.quality.reason = "context restored: new scene (GPU state lost)";
        this.draw(); this.schedule();
      } catch (error) { this.fail(error); }
    }, { signal });
    // A pointer gesture boosts pacing. Injection remains an explicit command with a known dose.
    canvas.addEventListener("pointerdown", () => { this.boostedUntil = performance.now() + 2000; }, { signal });
    canvas.addEventListener("pointermove", e => { if (e.buttons) this.boostedUntil = performance.now() + 2000; }, { signal });
    this.resizeObserver = new ResizeObserver(() => {
      try { if (!this.lost && !this.disposed) { this.resize(); this.draw(); } } catch (error) { this.fail(error); }
    });
    this.resizeObserver.observe(canvas);
    if (typeof IntersectionObserver !== "undefined") {
      this.observer = new IntersectionObserver(entries => { this.offscreen = !entries[0]?.isIntersecting; this.schedule(); });
  ...
  private initialize(): void {
    ...
    if (!this.vao || !this.buffer) throw new Error("Liquid geometry allocation failed");
  ...
  private aspect(): number { return Math.max(1, this.canvas.clientWidth) / Math.max(1, this.canvas.clientHeight); }
  ...
  private resize(): void {
    ...
    this.resizeCanvas(); this.effectStart = performance.now();
  }
  private draw(): void {
    this.gl.bindVertexArray(this.vao);
    const fade = Math.min(1, (performance.now() - this.effectStart) / 250);
    const now = performance.now(), target = this.scene.detail && LIQUID_TIERS[this.quality.tier].detail ? 1 : 0;
    const delta = Math.min(1, (now - this.lastDraw) / 250); this.lastDraw = now;
    this.detailAmount += Math.sign(target - this.detailAmount) * Math.min(Math.abs(target - this.detailAmount), delta);
    this.timing.measure("rendererMs", () => this.display.draw(this.solver.phaseRead, this.solver.pigmentRead, this.scene, this.canvas.width, this.canvas.height, this.diagnosticColors, fade, this.solver.detailRead, this.solver.velocityRead, this.solver.simulatedTime, this.detailAmount));
  }
  private loop = (now: number): void => {
    ...
    try {
      ...
      if (now - this.presented >= 1000 / fps - 1) {
        ...
        this.gl.bindVertexArray(this.vao);
        this.timing.poll();
        this.timing.measure("solverMs", () => this.clock.advance(() => this.solver.stableDt, dt => this.solver.step(dt)));
        this.draw();
  ...
  play(): void { this.paused = false; this.schedule(); }
  pause(): void { this.paused = true; this.stop(); }
  reset(): void {
    this.gl.bindVertexArray(this.vao);
    const next = new LiquidSolver(this.gl, this.scene, this.solver.width, this.solver.height);
    this.solver.dispose(); this.solver = next; this.clock = new LiquidClock(); this.draw();
  }
  getScene(): LiquidScene { return structuredClone(this.scene); }
  ...
  inject(x: number, y: number, slot = 0, fraction = .1, radius = .04, vx = 0, vy = 0): void {
    ...
    this.gl.bindVertexArray(this.vao); this.solver.inject(x, y, slot, fraction, radius, vx, vy);
    this.boostedUntil = performance.now() + 2000; this.draw();
  }
  diagnostics(): LiquidDiagnostics { this.gl.bindVertexArray(this.vao); return this.solver.diagnostics(); }
  ...
  diagnosticStep(seconds: number): void {
    if (!this.paused || !Number.isFinite(seconds) || seconds <= 0 || seconds > 1) throw new Error("Pause and request at most one second per diagnostic batch");
    this.gl.bindVertexArray(this.vao);
    let remaining = seconds;
    while (remaining > 1e-9) { const dt = Math.min(this.solver.stableDt, remaining); this.solver.step(dt); remaining -= dt; }
    this.draw();
  }
  ...
  private release(): void {
    ...
    this.resourcesValid = false;
    this.solver?.dispose(); this.display?.dispose(); this.timing?.dispose(); this.gl.deleteBuffer(this.buffer); this.gl.deleteVertexArray(this.vao);
  }
  dispose(): void {
    if (this.disposed) return; this.disposed = true; this.stop(); this.abort.abort();
    this.resizeObserver?.disconnect(); this.observer?.disconnect(); LiquidEngine.instances.delete(this); this.release();
  }
}
