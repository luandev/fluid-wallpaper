...
export function transportLimit(h: number, maxFaceSpeed: number): { gamma: number; dt: number; epsilon: number } {
 ...
 */
export function carrierRates(velocity: number, left: number, right: number, normalLeft: number, normalRight: number, gamma: number): [number, number] {
  return [gamma + 0.5 * (velocity + gamma * normalLeft * (1 - left)),
    gamma - 0.5 * (velocity + gamma * normalRight * (1 - right))];
}

export function areaGrid(area: number, aspect: number): { width: number; height: number } {
  if (!Number.isFinite(aspect) || aspect <= 0 || !Number.isFinite(area) || area < 64) throw new Error("Invalid grid budget");
...
export function conservativeResample(data: ArrayLike<number>, width: number, height: number, nextWidth: number, nextHeight: number, channels: number): Float32Array {
  const out = new Float32Array(nextWidth * nextHeight * channels);
  for (let y = 0; y < nextHeight; y++) for (let x = 0; x < nextWidth; x++) {
    const x0 = x * width / nextWidth, x1 = (x + 1) * width / nextWidth;
    const y0 = y * height / nextHeight, y1 = (y + 1) * height / nextHeight;
    for (let j = Math.floor(y0); j < Math.ceil(y1); j++) for (let i = Math.floor(x0); i < Math.ceil(x1); i++) {
      const weight = Math.max(0, Math.min(x1, i + 1) - Math.max(x0, i)) * Math.max(0, Math.min(y1, j + 1) - Math.max(y0, j)) / ((x1 - x0) * (y1 - y0));
      for (let c = 0; c < channels; c++) out[(y * nextWidth + x) * channels + c] += data[(j * width + i) * channels + c] * weight;
    }
  }
  return out;
}
