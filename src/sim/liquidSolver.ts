...
export class LiquidSolver {
  ...
  constructor(
    ...
    if (!gl.getExtension("EXT_color_buffer_float"))
      throw new Error(
        "Two-liquid diagnostics require EXT_color_buffer_float (RGBA32F)",
      );
    if (width * height * 16 * 8 > LIQUID_MEMORY_CAP)
      throw new Error("Liquid allocation exceeds 32 MiB cap");
    this.h = 1 / Math.min(width, height);
    this.program = compileProgram(gl, vertexSource, liquidSource, "two-liquid");
    const allocate = () => {
      const f = createFbo(gl, width, height, {
    ...
    try {
      this.phase = allocate();
      this.pigment = allocate();
      this.velocity = allocate();
      this.pressure = allocate();
      this.scratch = allocate();
      this.rhs = allocate();
      this.divergence = allocate();
      ...
      if (seed) {
        this.pass(0, this.phase);
        this.pass(1, this.pigment);
        this.pass(12, this.detail);
      }
    } catch (error) {
      this.dispose();
      throw error;
  ...
  get stableDt(): number {
    return Math.min(
      1 / 30,
      transportLimit(this.h, this.speed).dt,
      capillaryLimit(this.h, this.scene.surfaceTension),
    );
  }
  get allocationBytes(): number {
    return this.width * this.height * 16 * this.targets.length;
  }
  ...
  private pass(mode: number, target: FBO, dt = 0, source = this.phase): void {
    ...
    ["uField", "uVelocity", "uPhase", "uPressure", "uRhs"].forEach(
      (name, i) => {
        const input =
          inputs[i] === target
    ...
    gl.uniform2i(this.loc("uSize"), this.width, this.height);
    gl.uniform1i(this.loc("uResetBank"), this.resetBank);
    const scalars: Record<string, number> = {
      uH: this.h,
      uDt: dt,
      uGamma: transportLimit(this.h, this.speed).gamma,
      uSigma: this.scene.surfaceTension,
      uDamping: this.scene.damping,
      uStirring: this.scene.stirring,
      uScale: this.scene.swirlScale,
      uTime: this.time,
      uDose: this.scene.dose,
    };
    ...
    gl.uniform2f(
      this.loc("uViscosity"),
      this.scene.phases[0].viscosity,
      this.scene.phases[1].viscosity,
    );
    gl.uniform4fv(
      this.loc("uCarriers"),
      this.scene.pigments.map((p) => p.phase),
    );
    gl.drawArrays(gl.TRIANGLES, 0, 3);
    this.passCount++;
  }
  private swap(
    key: "phase" | "pigment" | "velocity" | "pressure" | "detail",
  ): void {
    const old = this[key];
    this[key] = this.scratch;
    this.scratch = old;
  }
  read(target: FBO): Float32Array {
    ...
    if (this.gl.getError() !== this.gl.NO_ERROR)
      throw new Error("Liquid GPU readback failed");
    return values;
  }
    ...
    for (let i = 0; i < values.length; i += 4)
      for (let c = 0; c < channels; c++) {
        if (!Number.isFinite(values[i + c]))
          throw new Error("Nonfinite liquid state");
    ...
    for (let batch = 0; batch < 40; batch++) {
      ...
      for (let y = 0; y < this.height; y++)
        for (let x = 0; x < this.width; x++) {
          const k = 4 * (y * this.width + x);
          let laplacian = 0;
          if (x > 0) laplacian += p[k - 4] - p[k];
          if (x < this.width - 1) laplacian += p[k + 4] - p[k];
          if (y > 0) laplacian += p[k - this.width * 4] - p[k];
          if (y < this.height - 1) laplacian += p[k + this.width * 4] - p[k];
          this.pressureResidual = Math.max(
            this.pressureResidual,
            Math.abs(laplacian / this.h ** 2 - rhs[k]),
          );
        }
      if (this.pressureResidual <= 1e-5) break;
    }
    this.pass(6, this.scratch);
    this.swap("velocity");
    this.speed = this.max(this.read(this.velocity), 2);
  }
  step(dt: number): void {
    if (!(dt > 0) || dt > this.stableDt * (1 + 1e-6))
      throw new Error("Unstable requested liquid timestep");
    this.pass(2, this.scratch, dt);
    this.swap("velocity");
    ...
    const diagonal =
      1 +
      (12 * Math.max(...this.scene.phases.map((p) => p.viscosity)) * dt) /
        this.h ** 2;
    for (let batch = 0; batch < 32; batch++) {
      for (let i = 0; i < 15; i++) {
        this.pass(3, this.scratch, dt);
        this.swap("velocity");
      ...
      this.swap("velocity");
      this.viscosityIterations++;
      const next = this.read(this.velocity);
      let difference = 0;
      for (let i = 0; i < next.length; i += 4)
        for (let c = 0; c < 2; c++)
          difference = Math.max(
            difference,
            Math.abs(next[i + c] - previous[i + c]),
          );
    ...
    if (dt > transportLimit(this.h, this.speed).dt * (1 + 1e-6)) {
      throw new Error(
        "Post-projection speed exceeded transport limit; reduce forcing/timestep",
      );
    }
    this.transport(dt);
    this.lastDt = dt;
  ...
  transport(dt: number): void {
    if (!(dt > 0) || dt > transportLimit(this.h, this.speed).dt * (1 + 1e-6))
      throw new Error("Unstable transport timestep");
    // Pigment must see the same OLD carrier field and projected velocity as phase transport.
    this.pass(8, this.scratch, dt, this.pigment);
    this.swap("pigment");
    this.pass(7, this.scratch, dt, this.phase);
    this.swap("phase");
    ...
    this.pass(13, this.scratch, dt, this.detail);
    this.swap("detail");
    this.resetBank = -1;
    this.time += dt;
  }
    ...
    for (let y = 0; y < this.height; y++)
      for (let x = 0; x < this.width; x++) {
        ...
        velocity[i] =
          x === this.width - 1
            ? 0
            : (psi((x + 1) / this.width, (y + 1) / this.height) -
                psi((x + 1) / this.width, y / this.height)) /
              this.h;
        velocity[i + 1] =
          y === this.height - 1
            ? 0
            : -(
                psi((x + 1) / this.width, (y + 1) / this.height) -
                psi(x / this.width, (y + 1) / this.height)
              ) / this.h;
      }
    this.upload(this.velocity, velocity);
    this.speed = this.max(velocity, 2);
  }
  diagnostics(): LiquidDiagnostics {
    const a = this.read(this.phase),
      c = this.read(this.pigment);
    this.pass(4, this.divergence);
    let min = Infinity,
      max = -Infinity,
      sum = 0,
    ...
    for (let i = 0; i < a.length; i += 4) {
      ...
      sum += a[i];
      if (a[i] > 0.9) {
        insidePressure += pressure[i];
        insideCount++;
      }
      if (a[i] < 0.1) {
        outsidePressure += pressure[i];
        outsideCount++;
      }
      for (let k = 0; k < 4; k++) {
        sums[k] += c[i + k];
        pigmentMin = Math.min(pigmentMin, c[i + k]);
      }
    }
    return {
      phaseMin: min,
      phaseMax: max,
      phaseMean: sum / (a.length / 4),
      pigmentMeans: sums.map((v) => v / (a.length / 4)),
      pigmentMin,
      maxVelocity: this.max(this.read(this.velocity), 2),
      maxDivergence: this.max(this.read(this.divergence), 1),
      ...
      viscosityIterations: this.viscosityIterations,
      pressureResidual: this.pressureResidual,
      viscosityResidualBound: this.viscosityResidualBound,
      pressureJump:
        this.lastDt && insideCount && outsideCount
          ? (insidePressure / insideCount - outsidePressure / outsideCount) /
            this.lastDt
          : 0,
    };
  }
  ...
  resize(width: number, height: number): LiquidSolver {
    if (this.allocationBytes + width * height * 16 * 8 > LIQUID_MEMORY_CAP)
      throw new Error("Transition allocation exceeds 32 MiB");
    const next = new LiquidSolver(this.gl, this.scene, width, height, false);
    try {
      for (const key of ["phase", "pigment", "velocity", "detail"] as const)
        next.upload(
          next[key],
          conservativeResample(
            this.read(this[key]),
  ...
  inject(
    ...
    if (
      ![x, y, fraction, radius, vx, vy].every(Number.isFinite) ||
      ...
      throw new Error("Invalid liquid injection");
    const phase = this.read(this.phase),
      pigment = this.read(this.pigment),
      velocity = this.read(this.velocity);
    const carrier = this.scene.pigments[slot].phase;
    for (let j = 0; j < this.height; j++)
      for (let i = 0; i < this.width; i++) {
        const k = 4 * (j * this.width + i);
        const dx = ((i + 0.5) / this.width - x) * this.width * this.h;
        const dy = ((j + 0.5) / this.height - y) * this.height * this.h;
        const f =
          fraction * Math.exp(-(dx * dx + dy * dy) / (2 * radius * radius));
        ...
        velocity[k] += f * Math.max(-0.1, Math.min(0.1, vx));
        velocity[k + 1] += f * Math.max(-0.1, Math.min(0.1, vy));
        if (i === this.width - 1) velocity[k] = 0;
        if (j === this.height - 1) velocity[k + 1] = 0;
      }
    this.upload(this.phase, phase);
    this.upload(this.pigment, pigment);
    this.upload(this.velocity, velocity);
    this.project();
  }
  dispose(): void {
    this.targets.forEach((f) => deleteFbo(this.gl, f));
    this.targets = [];
