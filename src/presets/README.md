...
The landing imports three existing IDs from here; docs and heroes share this same collection. See [contribution guide](../../docs/contributing-presets.html). Landing showcase configs stay independently authored in `src/landing/scenes.ts` so the home page is not bound to docs-hero presentation (lower grid, slow time, close crop).
