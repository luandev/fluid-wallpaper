...
const QUALITY_PATCHES: Record<
  Exclude<FluidInkQuality, "auto">,
  Partial<FluidConfig>
> = {
...
const STYLE = `
:host { display:block; position:relative; width:100%; height:100%; min-height:240px; overflow:hidden; background:transparent; }
...
export class FluidInkElement extends ElementBase {
  ...
  constructor() {
    ...
    if (
      ...
      throw new Error(
        "fluid-ink requires a browser DOM to construct an element.",
      );
    }
  ...
  connectedCallback(): void {
    ...
    try {
      ...
      if (this.pendingScene) {
        ...
        if (quality && ["auto", "eco", "balanced", "high"].includes(quality))
          this.pendingScene.quality = quality as FluidInkQuality;
        this.liquid = new LiquidEngine(
          this.canvas,
          this.pendingScene,
          (status) =>
            this.dispatchEvent(
              new CustomEvent("qualitychange", { detail: status }),
            ),
      ...
      if (quality === "balanced" || quality === "high")
        Object.assign(config, QUALITY_PATCHES[quality]);
      ...
      this.dispatchEvent(
        new CustomEvent("qualitychange", {
          detail: this.engine.getEcoStatus() ?? { quality },
        }),
      );
      ...
      this.dispatchEvent(
        new CustomEvent("ready", { bubbles: false, composed: false }),
      );
    } catch (error) {
      this.engine?.dispose();
      this.engine = null;
  ...
  attributeChangedCallback(name: string): void {
    if (this.liquid) {
      if (name === "paused") {
        if (this.hasAttribute("paused")) this.liquid.pause();
        else this.liquid.play();
      }
      if (name === "quality") {
        const next = this.liquid.getScene();
        const quality = this.getAttribute("quality") ?? "auto";
        if (["auto", "eco", "balanced", "high"].includes(quality)) {
          next.quality = quality as FluidInkQuality;
          this.updateScene(next);
        }
      }
      return;
    }
  ...
  set config(next: Partial<FluidConfig>) {
    if (this.pendingScene)
      throw new Error("Use updateScene for the two-liquid model");
  ...
  inject(command: FluidInkInjection): void {
    ...
    if (this.liquid) {
      this.liquid.inject(
        clamp01(command.position.x),
        clamp01(command.position.y),
        command.pigmentSlot ?? 0,
        command.fraction ?? 0.1,
        command.radius ?? 0.04,
        velocity.x,
        velocity.y,
      );
  ...
  setConfig(patch: Partial<FluidConfig>): void {
    if (this.pendingScene)
      throw new Error(
        "Use updateScene for the two-liquid model; legacy sliders have no physical migration",
      );
  ...
  get scene(): LiquidScene | null {
    return (
      this.liquid?.getScene() ??
      (this.pendingScene ? structuredClone(this.pendingScene) : null)
    );
  }
  ...
  updateScene(value: LiquidScene): void {
    if (!this.liquid)
      throw new Error("Assign a scene on a connected element first");
    this.liquid.updateScene(value);
    this.pendingScene = this.liquid.getScene();
    this.dispatchEvent(new CustomEvent("scenechange", { detail: this.scene }));
  }
  get qualityStatus(): LiquidStatus | EcoStatus | null {
    return this.liquid?.status ?? this.engine?.getEcoStatus() ?? null;
  }
  ...
  private applyQuality(): void {
    ...
    if (quality !== "auto") {
      const next = this.engine.applyConfig(
        QUALITY_PATCHES[quality] ?? QUALITY_PATCHES.balanced,
      );
      this.dispatchEvent(new CustomEvent("configchange", { detail: next }));
    }
    ...
    if (!wasEco)
      this.dispatchEvent(
        new CustomEvent("qualitychange", { detail: { quality } }),
      );
  }
  ...
  private showError(message: string): void {
    ...
    this.dispatchEvent(
      new CustomEvent("error", {
        detail: { code: "ENGINE_INIT_FAILED", message },
      }),
    );
  }
}
