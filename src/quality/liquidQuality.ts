...
export class LiquidQualityController {
  ...
  get simulationSpeed(): number {
    return this.policy === "eco" ? this.eco.budget.speed : 1;
  }
  get cellBudget(): number {
    return this.policy === "eco"
      ? Math.round(16384 * this.eco.budget.scale ** 2)
      : LIQUID_TIERS[this.tier].cells;
  }
  get pixelBudget(): number {
    return this.policy === "eco"
      ? Math.min(262144, this.eco.budget.pixels)
      : LIQUID_TIERS[this.tier].pixels;
  }
  suspend(): void {
    this.eco.suspend();
    this.samples = [];
  ...
  sample(workMs: number, pacingOverload: boolean): void {
    ...
    if (Number.isFinite(workMs) && workMs >= 0)
      this.samples.push({ workMs, pacingOverload });
  ...
  observe(
    now: number,
    workMs: number,
    pacingOverload: boolean,
    presentationFps: number,
  ): boolean {
    if (this.policy === "eco") {
      const changed = this.eco.observe(
        now,
        workMs,
        (1000 / this.fps) * (pacingOverload ? 2 : 1),
      );
    ...
    if (this.samples.length) {
      ...
      workMs = sorted[Math.floor((sorted.length - 1) * 0.9)];
      pacingOverload =
        this.samples.filter((s) => s.pacingOverload).length /
          this.samples.length >
        0.2;
      this.samples = [];
    }
    ...
    this.overloaded =
      workMs > budget || pacingOverload ? this.overloaded + 1 : 0;
    this.comfortable =
      workMs < budget * 0.6 && !pacingOverload ? this.comfortable + 1 : 0;
...
export class LiquidClock {
  ...
  add(seconds: number, speed = 1): void {
    ...
    if (this.pending > 4 / 30) {
      this.dropped += this.pending - 4 / 30;
      this.pending = 4 / 30;
    }
  }
