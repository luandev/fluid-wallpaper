...
export interface LiquidTiming { solverMs: number; rendererMs: number; source: "gpu" | "cpu-submission-proxy" }
...
export class GpuTiming {
  ...
  value: LiquidTiming = { solverMs: 0, rendererMs: 0, source: "cpu-submission-proxy" };
  ...
  measure(label: "solverMs" | "rendererMs", run: () => void): void {
    ...
    try { run(); } finally {
      if (query && ext) { gl.endQuery(ext.TIME_ELAPSED_EXT); this.pending.push({ query, label }); }
      if (this.value.source !== "gpu") this.value[label] = performance.now() - start;
    }
  }
  poll(): void {
    ...
    if (gl.getParameter(ext.GPU_DISJOINT_EXT)) {
      this.pending.forEach(p => gl.deleteQuery(p.query)); this.pending = [];
      this.value = { solverMs: 0, rendererMs: 0, source: "cpu-submission-proxy" }; return;
    }
    this.pending = this.pending.filter(p => {
      if (!gl.getQueryParameter(p.query, gl.QUERY_RESULT_AVAILABLE)) return true;
      this.value[p.label] = Number(gl.getQueryParameter(p.query, gl.QUERY_RESULT)) / 1e6;
      this.value.source = "gpu"; gl.deleteQuery(p.query); return false;
    });
  }
  dispose(): void { this.pending.forEach(p => this.gl.deleteQuery(p.query)); this.pending = []; }
}
