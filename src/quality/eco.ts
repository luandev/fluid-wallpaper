...
export const ECO_LEVELS = [
  ...
  { scale: 0.75, fps: 30, speed: 0.85, pixels: 294912 },
  { scale: 0.5, fps: 24, speed: 0.65, pixels: 131072 },
  { scale: 1 / 3, fps: 20, speed: 0.5, pixels: 65536 },
] as const;
...
export class EcoController {
  ...
  observe(now: number, workMs: number, presentationMs: number): boolean {
    if (
      ![now, workMs, presentationMs].every(Number.isFinite) ||
      workMs < 0 ||
      presentationMs <= 0
    )
      return false;
    ...
    const p90 = work[Math.floor((work.length - 1) * 0.9)];
    const late =
      this.samples.filter((s) => s.pacing > interval * 1.6).length /
        this.samples.length >
      0.2;
    this.samples = [];
    this.overloaded = p90 > interval * 0.7 || late ? this.overloaded + 1 : 0;
    this.comfortable = p90 < interval * 0.4 && !late ? this.comfortable + 1 : 0;
...
export function ecoGrid(config: FluidConfig, level: number) {
  ...
  return {
    simResolution: Math.max(
      64,
      Math.round(Math.min(config.simResolution, 192) * budget.scale),
    ),
    dyeResolution: Math.max(
      128,
      Math.round(Math.min(config.dyeResolution, 384) * budget.scale),
    ),
