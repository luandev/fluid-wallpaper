...
Follow-up 2026-09-15 at owner request: add a sixth public docs page ([shaders.html](shaders.html)) and durable notes ([SHADERS.md](SHADERS.md)) explaining the Navier–Stokes problem in plain terms and the Stam / GPU Gems ch. 38 method family used by the WebGL2 passes. No change to simulation authority or provenance policy.

## DEC-019 - Multi-host HTML wallpaper pack

Accepted 2026-09-17 at owner request. Ship one shared offline WebGL2 wallpaper entry (`wallpaper.html` + `src/wallpaper/`) that reuses landing scene selection and ECO defaults, without React tuner chrome, font CDNs, or media. Pack from `dist/` with minimal Wallpaper Engine `project.json` (`type: "web"`, empty properties) and Lively `LivelyInfo.json` plus preview stills via `yarn pack:wallpaper` → versioned zip. Pause continues to use existing `BrowserPlatform` visibility; do not add `window.wallpaper*` or Lively property listeners in this slice. Document install for Windows (WE + Lively), Linux WE where available, and macOS HTML hosts. Zip packaging is validated in-repo; host GPU / Workshop / audio / user properties remain open ([OPEN_QUESTIONS.md](OPEN_QUESTIONS.md)).
